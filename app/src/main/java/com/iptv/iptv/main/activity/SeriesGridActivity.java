...
public class SeriesGridActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        client.get(ApiUtils.appendUri(ApiUtils.SERIES_FILTER_URL, ApiUtils.addToken()),
                new TextHttpResponseHandler() {
                    @Override
                    public void onFailure(int statusCode, Header[] headers, String responseString,
                            Throwable throwable) {
                        category.add(new CategoryItem(-1, "ซีรี่ย์มาใหม่", -1));
                        category.add(new CategoryItem(-1, "ซีรี่ย์ยอดนิยม", -1));
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                EventBus.getDefault().post(new LoadSeriesEvent(
                        ApiUtils.appendUri(ApiUtils.SERIES_URL, ApiUtils.addToken())));
            }
        }, 500);
            ...
            public void onClick(View view) {
                getFragmentManager().beginTransaction().replace(R.id.layout_filter,
                        FilterFragment.newInstance(
                                ApiUtils.appendUri(ApiUtils.SERIES_FILTER_URL, ApiUtils.addToken()),
    ...
    public void onFilterEvent(ApplyFilterEvent event) {
        if (event.isApplied) {
            String url = ApiUtils.SERIES_URL;
    ...
    public void onSelectMenuEvent(SelectMenuEvent event) {
        if (event.position > 1 && event.position < category.size() - 2) {
            ...
            if (event.position == 0) {
                ...
                PrefUtils.setBooleanProperty(R.string.pref_current_favorite, false);
                EventBus.getDefault().post(new LoadSeriesEvent(
                        ApiUtils.appendUri(ApiUtils.SERIES_HISTORY_URL, ApiUtils.addToken())));
                clearFilter();
