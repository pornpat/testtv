...
public class EasyVideoPlayer extends FrameLayout implements IUserMethods, TextureView.SurfaceTextureListener,
        MediaPlayer.OnPreparedListener, MediaPlayer.OnInfoListener, MediaPlayer.OnBufferingUpdateListener, MediaPlayer.OnCompletionListener,
    ...
    public void setTrack(int position) {
        mMediaPlayer.selectTrack(position);
    }
...
//                    && trackInfoArray[i].getLanguage().equals(Locale.getDefault().getISO3Language())) {
                mTrackList.add(trackInfoArray[i].getLanguage());
            }
        }
        if (mCallback != null) {
            mCallback.onInfo(this);
        }
